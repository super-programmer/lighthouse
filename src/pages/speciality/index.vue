  ...
  <view class="min-h-screen bg-gray-100">
    ...
    <view class="container mx-auto px-4 py-4">
      ...
      <view v-if="loading" class="text-center py-10">
        <view class="w-8 h-8 border-4 border-blue-200 border-t-blue-500 rounded-full animate-spin mx-auto"></view>
      ...
      <view v-else class="grid grid-cols-1 gap-4">
        <view 
          class="bg-white rounded-lg shadow-sm p-4 mb-4 transition-all duration-300 hover:shadow-md"
          v-for="major in allSpeciality" 
          :key="major.zyId"
      ...
      <!-- 分页加载 -->
      <view v-if="!loading && filteredSpeciality.length > 0 && hasMore" class="text-center py-4">
        <button 
          class="border border-blue-500 text-blue-500 px-6 py-2 rounded-full text-sm"
          @click="loadMore"
        >
          加载更多
        </button>
...
import { getSpecialityByPage, searchSpeciality } from '@/api/speciality.js' // 假设新增的API
  ...
  methods: {
    ...
    async loadMore() {
      ...
      try {
        ...
        const newSpeciality = res.data.list
